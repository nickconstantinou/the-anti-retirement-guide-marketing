...
export default function Jumpstart7Day() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      if (typeof window !== 'undefined' && window.posthog) {
        window.posthog.capture('email_signup', { email: email, source: 'cluster-d' })
      }
      ...
      <section className="py-16 sm:py-20 px-4">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <p className="text-lg sm:text-xl text-slate-300 mb-6 max-w-xl mx-auto">
            You've run the numbers. They work — or close enough that it shouldn't be the thing stopping you.
            But every morning, your alarm goes off at 6am anyway.
          </p>
          <p className="text-base text-slate-400 mb-8">
            That's not a money problem. That's a "what am I actually for?" problem.
          </p>
        </div>
      </section>

      {/* What's Inside - From Ad Copy */}
      <section className="py-12 px-4 bg-slate-800/50">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 sm:p-8">
            <ul className="space-y-4">
              {[
                ...
                ['Day 7', 'The decision — name what\'s actually stopping you'],
              ].map(([day, desc], i) => (
          ...
          <p className="text-center text-slate-400 mt-6 text-sm">
            Not journaling prompts. Not vision boards. Specific questions that expose what you're actually avoiding.
          </p>
        </div>
      </section>
